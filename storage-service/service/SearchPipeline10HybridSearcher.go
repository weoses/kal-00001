...
func (s HybridSearcher) Search(ctx context.Context, accountIds []uuid.UUID, query string, metadataType *entity.MetadataType, sortKey entity.ElasticSortKey, size int) ([]*entity.ElasticImageMetaData, entity.ElasticSortKey, error) {
	if query == "" {
	...
	results, nextKey, err := s.metadata.SearchHybridOrderByScore(
		ctx, accountIds, query, *embedding, s.searchConfig.Fuzziness, metadataType, sortKey, size,
	)
	if err != nil {

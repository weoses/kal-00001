...
type SearchService interface {
	Search(ctx context.Context, accountIds []uuid.UUID, query string, metadataType *entity.MetadataType, afterId *PipelineAfterID, size int) (*SearchServiceResponse, error)
}
...
func (m *SearchServiceImpl) Search(
	...
	for _, searcher := range m.searchers {
		...
		data, returnedKey, err := searcher.Search(ctx, accountIds, query, metadataType, sortKey, size)

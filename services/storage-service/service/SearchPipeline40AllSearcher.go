...
func (a AllSearcher) Search(ctx context.Context, accountIds []uuid.UUID, query string, metadataType *entity.MetadataType, sortKey entity.ElasticSortKey, size int) ([]*entity.ElasticImageMetaData, entity.ElasticSortKey, error) {
	if query != "" {
		return []*entity.ElasticImageMetaData{}, nil, nil
	}
	a.slogger.InfoContext(ctx, "search", "query", query, "sortKey", sortKey, "size", size)

	results, nextKey, err := a.metadata.GetByAccountIdOrderByCreated(ctx, accountIds, metadataType, sortKey, size)
	if err != nil {
